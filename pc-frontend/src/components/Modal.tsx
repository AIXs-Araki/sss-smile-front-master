...
export function Modal(props: PropsWithChildren<{ customStyle?: React.CSSProperties, open: boolean, setOpen: (o: boolean) => void, renderButtons?: () => ReactNode, maskClosable?: boolean }>) {
  ...
  return (
    <ModalFrame customStyle={customStyle} opened={open} close={() => setOpen(false)} maskClosable={typeof props.maskClosable === 'boolean' ? props.maskClosable : true} rounded>
      <ModalContent>
        {props.children}
      </ModalContent>
...
export function NonclosableModal(props: PropsWithChildren<{ customStyle?: React.CSSProperties, open: boolean }>) {
  ...
  return (
    <ModalFrame customStyle={customStyle} opened={open} maskClosable={false} close={() => { }} rounded>
      <ModalContent>
        {props.children}
      </ModalContent>
    </ModalFrame >
...
export function ClosableModal(props: PropsWithChildren<ClosableModalProps>) {
  ...
  return (
    <ModalFrame customStyle={customStyle} opened={open} close={close} maskClosable={true} className={props.framePanelClassName} rounded>
...
export function LargeClosableModal(props: PropsWithChildren<ClosableModalProps>) {
  ...
  return (
    <ModalFrame customStyle={customStyle} opened={open} close={close} maskClosable={true} className={frameClass} rounded>
      <ModalHeader close={close} >
        {props.title}
      </ModalHeader>
      <ModalContent>
...
export function ModalHeader(props: PropsWithChildren<{ close: () => void }>) {
  return <div className="bg-white px-4 py-3 sm:px-6 flex flex-row print:hidden rounded-t-3xl">
    <div
      className="modal-header flex flex-shrink-0 justify-between p-4 border-b border-gray-200 w-full">
...
export function ModalContent(props: PropsWithChildren) {
  return <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 overflow-visible">{props.children}</div>
}

export function ModalButtons(props: PropsWithChildren) {
  return <div className="bg-gray-50 px-4 py-3 sm:px-6 flex justify-end print:hidden rounded-b-3xl">{props.children}</div>
}
...
export function ModalFrame(props: ModalFrameProps) {
  const className = twMerge(
    "inline-block align-bottom text-left shadow-xl transform transition-all ease-in-out sm:my-8 sm:align-middle sm:w-full sm:max-w-3xl text-xl relative",
    props.rounded && "before:absolute before:inset-0 before:rounded-3xl before:border before:border-gray-200 before:pointer-events-none",
    props.className
  );
