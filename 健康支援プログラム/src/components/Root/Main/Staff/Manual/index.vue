								...
								<div class="item target">
									<div class="info">
										<div class="label">
											対象者のID
											<span class="unit">
												<small>例: fixed-target-id-for-dev-1</small>
											</span>
										</div>
									</div>
...
export default {
	...
	data () {
		return {
			...
			target_id: 'fixed-target-id-for-dev-1',
			manuals: [
				{
					id: `date`,
					label: `入力値の時間`,
	...
	methods: {
		...
		setTouchEventConfirm (target) {
			...
			this.mc.on('panup panleft panright pandown tap', ev => {
				if (this.notify.target_id)
					return this.rumbleConfirm()
		...
		noneTargetID () {
			this.notify.target_id = this.$notify.error({
				title: `入力が足りません`,
				message: `対象者のIDを入力してください`,
				onClose: () => {
					this.notify.target_id = null
				},
				duration: 0 * 1000
			})
			_.delay(() => {
				if (this.notify.target_id)
					this.notify.target_id.close()
				this.notify.target_id = null
			}, 3 * 1000)
			this.rumbleConfirm()
		},
		rumbleConfirm () {
			this.confirm.rumble = true
			_.delay(() => {
				this.confirm.rumble = false
			}, 333)
		},
		// 表示関連 > スライダー
		// ========================
		...
		getManualValues () {
			...
			values.target_id = this.target_id
			values.data = []
			_.forEach(this.manuals, _d => {
				if (_d.id !== 'date') {
					const o = {}
